    ...
    methods: {
        ...
        async getInfo() {
            ...
            for (let i = 0; i < allUsers.length; i++) {
                let user = allUsers[i]
                let upline = await p.upline(user)
                ...
                let downline = await p.downline(user)
        ...
        async newBlock() {
            ...
            if (ctx && !this.edit) {
                ...
                while (currentUpdatesCount > this.updateIndex) {
                    ...
                    while (!success) {
                        try {
                            newUpdates = await p.getUpdates(this.updateIndex, updateBlockSize)
